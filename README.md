...
Use your own OpenAI-compatible provider and models inside the **Cursor** IDE.

> **This is NOT a free-model tool.** You must bring your own provider account and API key. The proxy simply routes Cursor's requests to your chosen endpoint, bypassing Cursor's built-in model restrictions.

---

## What problem does this solve?

Cursor has two limitations when you want to use a custom provider (OpenRouter, Bynara, DeepSeek, etc.):

1. **"Access to private networks is forbidden"** — Cursor blocks `localhost` / `127.0.0.1` as custom API endpoints. This tool spins up a public HTTPS tunnel so Cursor can reach the proxy.
...
- 🔌 **Zero Python dependencies** — uses only the standard library (`http.server`, `urllib`, `subprocess`).
- 🌐 **Stable public URL** — ngrok static domain support, so you configure Cursor once and never touch it again.
- 🔄 **Model-name translation** — add a custom prefix (e.g. `nry-`) to bypass Cursor's model validation.
- 🔒 **Proxy authentication** — `proxy_secret` prevents anyone with the public URL from burning your API credits.
- 🛡️ **Per-IP rate limiting** — token-bucket rate limit to blunt abuse if the URL leaks.
- 🎨 **Polished CLI** — ANSI banner, bordered panels, spinner, and colored request logs.
...
sequenceDiagram
    ...
    participant Tunnel as Public HTTPS Tunnel (ngrok)
    participant Proxy as Local Python Proxy (localhost:8080)
    participant API as Your Provider (OpenRouter, Bynara, ...)

    Cursor->>Tunnel: POST /v1/chat/completions (model: nry-claude-sonnet-4.6)
    Note over Tunnel: Secure tunnel forwards traffic to localhost
    Tunnel->>Proxy: POST /v1/chat/completions (model: nry-claude-sonnet-4.6)
    Note over Proxy: Strips prefix 'nry-' -> 'claude-sonnet-4.6'
    Proxy->>API: POST /v1/chat/completions (model: claude-sonnet-4.6)
    API-->>Proxy: Streams response
    Proxy-->>Tunnel: Streams response
    Tunnel-->>Cursor: Streams response
```
...
3. **Edit `config.json`**
   ...
   | `proxy_secret` | Secret Cursor must send as its "API Key". |
